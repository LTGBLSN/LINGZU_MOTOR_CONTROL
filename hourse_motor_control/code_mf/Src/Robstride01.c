...
// 定义参数索引列表
const uint16_t Index_List[15] = {0X7005, 0X7006, 0X700A, 0X700B, 0X7010, 0X7011, 0X7014, 0X7016, 0X7017, 0X7018, 0x7019, 0x701A, 0x701B, 0x701C, 0x701D};

uint32_t Mailbox; // 定义邮箱变量

/*******************************************************************************
* @功能     	: RobStride电机实例化的初始化函数
* @参数         : 电机结构体指针, CAN ID, MIT模式
* @返回值 		: void
* @概述  		: 初始化电机ID
*******************************************************************************/
...
void RobStride_Motor_Init_With_Offset(RobStride_Motor* motor, float (*Offset_MotoFunc)(float Motor_Tar), uint8_t CAN_Id, bool MIT_mode)
{
    motor->CAN_ID = CAN_Id;
    motor->Master_CAN_ID = 0xFD;
    motor->Motor_Set_All.set_motor_mode = move_control_mode;
...
* @参数2        : 参数地址
* @参数3        : 参数数值
* @参数4        : 选择是传入控制模式 还是其他参数 （Set_mode设置控制模式 Set_parameter设置参数）
* @返回值 		: void
* @概述  		: None
*******************************************************************************/
...
bool Get_MIT_Mode(RobStride_Motor* motor)
{
    return motor->MIT_Mode;
}

MIT_TYPE get_MIT_Type(RobStride_Motor* motor)
{
